...
export const useJoinAgency = () => {
    ...
    const [succmesg, setSucmsg] = useState(null)
        ...
        const joinAgency =  async (agencyname) => {
            ...
            try {
                ...
                if(storeditem !== null) {
                    ...
                    const response = await fetch(`${process.env.REACT_APP_API_URL}/user/joinagency`, {
                        method: 'PATCH',
                        headers: {
                    ...
                    if(response.ok) {
                        // console.log(json)
                        var updateUser = {...user, agency: agencyname, agDate: currentDate, }
                        dispatch(setUser(updateUser))
                        setSucmsg('joined successfully')
        ...
        return { joinAgency, loadingState, succmesg, error } 
}